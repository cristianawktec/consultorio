...
if($busca != "" and $busca != "todos"){

$busca2 = mysql_query("SELECT * FROM prospectos where prospecto_atendente_id = '$atendente' and prospecto_status = '1' and prospecto_nome LIKE '%$escola%' and prospecto_responsavel LIKE '%$responsavel%' and prospecto_email LIKE '%$email%' ORDER BY prospeto_historico_data DESC");
	
}

if($busca == "todos" and $data_id == "" and $data_f == ""){	
...
<title>LISTA DE PROSPECTOS - GERENCIAMENTO DE RELACIONAMENTO COM O CLIENTE EDUCA GENESIS [Vers&atilde;o 1.0]</title>
<style type="text/css">
...
<?php if($row3["prospecto_clienteagora"] == "1"){ ?>
  ...
  <?php }else{ ?>
    ...
    <td><a href="javascript:popUP('minhaJanela9', '<?php $_SERVER['PHP_SELF']; ?>?prospID=<?php echo $row3["prospecto_id"]; ?>', 'width=550,height=500,toolbar=no,menubar=no,status=no,scrollbars=no,resizable=yes')"><img src="images/emoticon_smile.png" width="20" style="padding-right: 5px;" alt="Enviar Para Clientes" title="Enviar para Clientes =)" border="0"/></a><?php if($row3["prospecto_clienteagora"] == "0"){ ?><a href="nao_cliente.php?prospID=<?php echo $row3["prospecto_id"]; ?>"><img src="images/emoticon_sad.png" width="20" title="Não deseja ser Cliente agora =(" border="0" /></a><?php } ?><a href="gera_pdf_prospecto.php?id=<?php echo $row3["prospecto_id"]; ?>"><img src="images/view.png" width="21" /></a><?php if($nivel == "1"){ ?><a href="<?php $_SERVER['PHP_SELF']; ?>?id=<?php echo $row3["prospecto_id"]; ?>&action=del" ><img src="images/icon_excluir.png" width="17" /></a><?php } ?></td>
...
<div id="rodape" style="font-family:Arial, Helvetica, sans-serif; font-size: 9px; position: absolute; top: 0px; right: 0px; color:#EEE;"><tr>
